"""Модуль с ответами на вопросы викторины по Git, VS Code и Python.

Этот модуль содержит ответы на вопросы из двух частей викторины:
- Часть 1: Алгоритм принятия и отдачи домашних заданий
- Часть 2: Настройка VS Code, перенос строк, линтеры, работа с ячейками

Модуль также включает практические примеры кода для демонстрации
различных способов переноса строк в Python.
"""
...
# %% [markdown]
# """
# Выполнение [TASK] Quiz #6 часть 1
# """

# %% [markdown]
# #### quiz1
#
# "Список вопросов к видео (https://t.me/c/1937296927/765) об алгоритме
# принятия и отдачи (проверки) домашних заданий."
#
# (По желанию) В ответе подробно всё опишите и обязательно нужно указывать
# таймкод из видео где я это сказал, по желанию, дополнительно прикладываем
# скриншот из видео. Если вы знаете ответы на вопросы из Вашего опыта,
# то таймкоды из видео не надо указывать и т.д.
#
# 1) Как понять, что домашка пришла?
# Ответ: в чате HomeWork должен появиться ник @Roslik и фразу: приму пул
#
# 2) Как принять домашку?
# Ответ: зайти в GitHub Desktop -> нажать кнопку Fetch origin ->
# посмотреть History
#
...
# - Compare to previous commit: Сравнивает текущий коммит с предыдущим,
#   показывая изменения.
# - Compare to working directory: Сравнивает коммит с текущим рабочим
#   каталогом.
# - Create branch from commit: Создает новую ветку, начиная с выбранного
#   коммита.
# - Reset to this commit: Переносит текущую ветку на выбранный коммит,
#   отменяя все последующие изменения.
# - Revert commit: Создает новый коммит, который отменяет изменения,
#   сделанные в выбранном коммите.
# - Cherry-pick commit: Применяет изменения из выбранного коммита в
#   текущую ветку.
...
# Ответ: по факту выполненного дз
#
# 5) Что такое интерпретатор?
# Ответ: - программа для запуска и чтения кода
#
...
#
# 10.1) Напишите список линтеров которые используются для проверки кода
...
# Ответ: - в GitHub при выведении error прописан линтер, который Failed
#          (также показывается файл, номер ячейки и ошибка)
#        - запуск терминал - ввод pre-commit run --all-files - красный(Failed)
#          - указание линтера
#
...
#        - линтер не считает markdown за ячейку
#
# 14) Где посмотреть номер ячейки в vscode?
# Ответ: - внизу справа cell
#
# 15) В каком формате ipynb отправляется в гитхаб? причём здесь JSON?
# Ответ: интерпретация формата в гитхаб
#
...
#
# 22) После внесения изменений в файлах, кнопка open in vs code пропадает
# в кошке, как по другому открыть vs code из кошки?
# Ответ: - правой кнопкой на репозиторий - open in vs code
#
# 23) Что такое stash?
# Ответ: это функция в Git, которая позволяет временно сохранять
# незавершённые изменения, не фиксируя их в репозитории
# Общее объяснение концепции: это временное хранилище изменений.
#
# 23.1) Как сохранить стэш?
# Ответ: git stash save "NAME_STASH" - сохранение изменения в рабочем
# каталоге с указанием имени
...
# Ответ: стэш - локальные изменения Пользователя, коммит - внешние
# изменения (четко зафиксированные)
#
...
#       - в history нажать на последний коммит(сохраняю изменения другого
#         и кидаю в буфер обмена свой)
...
# Ответ:
# - Пакетный менеджер — это инструмент, который автоматизирует процесс
#   установки, обновления, настройки и удаления программного обеспечения.
# - и conda и pip
# - conda
#
# 3) Почему расширение py лучше чем ipynb?
# Ответ: Использование .ipynb, когда вам нужна интерактивная среда,
# прототипирования, обучения и документирования.
#
# 4) Что такое pep8?
...
# %% [markdown]
# """
# Выполнение [TASK] Quiz #6 часть 2
# """

# %% [markdown]
# #### quiz2
#
# Список вопросов к видео https://t.me/c/1937296927/765/29781
...
# Ответ: This is a long string that we want to split across multiple lines.
#
...
# Ответ: This is a string that spans
# multiple lines. You can write freely
# and it will keep the line breaks.
#
...
# Ответ: This is a very long string that I would like to continue on
# the next line.
#
...
# Ответ: This is the first part, and this is the second part.
#
...
# Ответ:
# - ![alt text](image.jpg) при условии, что картинка в одной директории
#   с файлом
# - ![alt text](https://example.com/image.png) при размещении картинки
#   в сети интернет
#
...
# Ответ: Открыть файл Jupyter Notebook (.ipynb) -> клик на ячейку ->
# панель ячейки изменить на Markdown
#
# 11. Как запустить сразу все ячейки в юпитере?
# Ответ: Cell -> Run All
#
# 12. Как изменить размер картинки в юпитере? Нужно для этого знать HTML?
# Ответ: - через использование HTML-тега <img> :
#   <img src="" alt="альтернативный_текст" width="значение_ширины"
#   height="значение_высоты">
#
...
#
# 15. Что такое "Go To" чем это полезно? Как перейти сразу на ошибочную
# ячейку?
# Ответ:
# - Go To — функция, которая позволяет быстро перемещаться к определенной
#   строке, символу, определению функции или другой метке в коде.
# - кликнуть на строку в traceback -> автоматически перейдет к
#   соответствующей строке кода в ячейке.
#
...
# Ответ: Валидация — это процесс проверки данных на соответствие
# определенным правилам, ограничениям или требованиям.
#
# 23. Что такое трассировка ошибки?
# Ответ: Трассировка ошибки (Traceback) — это отчет, который генерирует
# интерпретатор или компилятор при возникновении ошибки во время
# выполнения программы.
#
# 24. Что значит отвалился интерпретатор?
# Ответ: неожиданно прекратил свою работу или вышел из строя во время
# выполнения программы.
#
